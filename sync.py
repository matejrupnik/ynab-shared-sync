...
def identify_sync_transactions(from_transactions, to_transactions, split):
    ...
    for transaction in to_transactions:
        ...
        for index, subtransaction in enumerate(subtransactions):
            if subtransaction.get("payee_name") and "Reimbursements".lower() in subtransaction.get(
                    "payee_name").lower() and subtransaction.get("amount") > 0:
        ...
        for index, subtransaction in enumerate(subtransactions):
            if subtransaction.get("payee_name") and "Reimbursements".lower() in subtransaction.get(
                ...
                if round(((100 - split) / 100) * transaction.get("amount"), -1) != split_amount:
                    print(
                        f"Transaction amount does not match split amount. {fingerprint} {round(((100 - split) / 100) * transaction.get('amount'), -1)}")
                    sys.exit(1)
    ...
    for index, subtransaction in enumerate(subtransactions):
        if subtransaction.get("payee_name") and "Reimbursements".lower() in subtransaction.get("payee_name").lower():
            ...
            mirrored_transaction = {
                ...
                "cleared": "cleared",
                "approved": True,
                "memo": memo + ("" if to_payee_id else main_subtransaction.get("payee_name") if main_subtransaction.get(
                    "payee_name") else "NoPayee") + ("" if to_category_id else main_subtransaction.get(
                    "category_name") if main_subtransaction.get("category_name") else "NoCategory"),
                "subtransactions": [
                    {
...
def sync_budgets():
    ...
    person1_sync_transactions = identify_sync_transactions(person1_transactions, person2_transactions,
                                                           int(PERSON1_SPLIT))
    person2_sync_transactions = identify_sync_transactions(person2_transactions, person1_transactions,
    ...
    if person1_mirrored_transactions:
        ...
        response = requests.post(url, headers=get_headers(PERSON2_API_KEY),
                                 json={"transactions": person1_mirrored_transactions})
    ...
    if person2_mirrored_transactions:
        ...
        response = requests.post(url, headers=get_headers(PERSON1_API_KEY),
                                 json={"transactions": person2_mirrored_transactions})
